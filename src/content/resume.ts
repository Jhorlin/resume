  ...
  experience: [
    {
      ...
      achievements: [
        ...
        "Solved multi-model support with the HTML5 playbook: feature detection plus polyfill. A probe harness tests every model across Amazon Bedrock, Mantle, OpenRouter, and Meta, using provider capability APIs where they suffice and running its own experiments where they don't (max-token rules that vary by endpoint, whether tool use and structured output can coexist, per-provider schema quirks), covering thinking modes, sampling rules, token accounting, and image, document, and video support.",
        "Polyfilled the gaps those probes reveal: a model lacking vision or document input is told its limitation in-prompt and handed policy tools (OCR, spreadsheet, and document readers) so it works around the gap with a tool call it makes knowingly. Where emulation would fabricate, as with structured output, the harness fails loud instead.",
